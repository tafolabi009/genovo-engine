#!/usr/bin/env python3
"""Regenerate the 4 missing files."""
import os

base = "C:/Users/USER/Downloads/game_engine/engine"

# Read the content we need to write from the generate scripts we already have
# Since the content was in the Write tool calls above, let's reconstruct them

# For efficiency, we'll create substantial implementations inline

world_v2 = '''//! Enhanced ECS World with archetype graph, column storage, typed resources,
...
# Write all 4 missing files
for path, content in [
    (f"{base}/ecs/src/world_v2.rs", world_v2),
]:
    with open(path, 'w', newline='\n') as f:
        f.write(content)
    print(f"Wrote {os.path.basename(path)} ({content.count(chr(10)) + 1} lines)")
